...
**Consigna 1:** Las últimas 3 cifras de mi DNI son 715. Crear una variable que tenga ese número.

```{r}
DNI=715
```

**Consigna 2:** Crear un vector que tenga todos los números desde el 1 hasta el 715.
```{r}
for(i in 1:715) 
print(1:i)
```
...
{python}
secuenciaDNI = range(1, 716)  # incluye 715
...
ggplot(df, aes(x = expr, y = time_ms, fill = expr)) +
  geom_violin(trim = FALSE, alpha = 0.7, color = "lightblue") +
  geom_boxplot(width = 0.2, fill = "black", outlier.shape = 16, outlier.size = 1) + 
  scale_y_log10() + 
